@extends('layouts.personal')
    @section('meta')
    <title>@lang('pe_leave.title')</title>
    <meta name="description" content="smart Control Horario my leave of absence, view my leave of absence records, edit pending leave request, and request leave of absence">
    @endsection
    ...
    <div class="container-fluid">
        <div class="row">
            <h2 class="page-title">@lang('pe_leave.page_title')
                <button class="ui positive button mini offsettop5 btn-add float-right"><i class="icon plus"></i>@lang('pe_leave.request')</button>
            </h2>
        </div>
            ...
            <div class="box box-success">
                <div class="box-body reportstable">
                    <form action="" method="get" accept-charset="utf-8" class="ui small form form-filter" id="filterform">
                        ...
                        <div class="inline two fields">
                            <div class="three wide field">
                                <label>@lang('pe_leave.date_range')</label>
                            ...
                            <button id="btnfilter" class="ui button positive small"><i class="ui icon filter alternate"></i> @lang('pe_leave.filter')</button>
                        </div>
                    </form>
                    ...
                    <table width="100%" class="table table-striped table-bordered table-hover delegation" id="dataTables-example">
                        <thead>
                            <tr>
                                <th>@lang('pe_leave.type')</th>
                                <th>@lang('pe_leave.from')</th>
                                <th>@lang('pe_leave.to')</th>
                                <th>@lang('pe_leave.reason')</th>
                                <th>@lang('pe_leave.return_date')</th>
                                <th>@lang('pe_leave.status')</th>
                                <th>@lang('pe_leave.action')</th>
                            </tr>
                        </thead>
                        <tbody>
                            @isset($l)
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($data->status == 'Approved')
                                        <button type="button" class="ui button icon mini teal view" data-id="{{ $data->id }}"><i class="ui icon file alternate"></i> @lang('pe_leave.view')</button>
                                    @else
                                        <a href="{{ url('personal/leaves/edit/'.$data->id) }}" class="ui blue icon mini basic button"><i class="ui icon edit"></i> @lang('pe_leave.edit')</a>
                                        <a href="{{ url('personal/leaves/delete/'.$data->id) }}" class="ui red icon mini basic button"><i class="ui icon trash"></i> @lang('pe_leave.delete')</a>
                                        @isset($data->comment)
